...
// 				</div>'
      ...
      row = row || { types:'drop', address:'', brief:'', domain: '' }
        ...
        area:"420px",
            ...
            !isEdit?{ type:'tips', list:['支持添加IP：如果添加多个IP请用","隔开','支持添加IP段,如：192.168.0.0/24','支持添加IP范围,格式如：192.168.1.xx-192.168.1.xx，暂不支持跨网段范围'] ,style:'padding-left: 40px; margin-top:5px;' }:{type:'tips',list:[]}
          ], row);
          bt_tools.setLayerArea(layero)
								  }
      })
	...
	ssh:{
		...
		loginLogsTable:function(param){
			...
			$.each(arry,function (i,v){
				html.append('<button type="button" class="btn btn-sm btn-'+ (logs[i][0] === param.select ?'success':'default') +'" data-type="'+ i +'">'+ v +'</button>')
			})
			return bt_tools.table({
				el: '#loginAllLogs',
				...
				success:function (config){
					$(config.config.el + ' .tootls_top .pull-right .cutLoginLogsType').remove()
			...
			bt.soft.get_soft_find('bt_security', function (rdata) {
				...
				if((rdata.setup && rdata.endtime > -1)) {
					...
					$('.installSoft .thumbnail-tab li').unbind('click').on('click',function(){
						var index = $(this).index()
						$(this).addClass('on').siblings().removeClass('on')
						console.log("index",index,$('.installSoft .thumbnail-item').eq(index).html())
										...
										success:function(){
											...
											function site_config_table(file_info) {
												...
												$.each(file_info,function (index,item){
													table_data.push({},{path: index},item)
												})
												bt_tools.table({
