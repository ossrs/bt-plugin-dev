...
var bt_file = {
  ...
  file_header: { 'file_checkbox': 40, 'file_name': 'auto', 'file_tamper_core': 80, 'file_accept': 120, 'file_size': 90, 'file_mtime': 145, 'file_ps': 'auto', 'file_operation': 350, 'file_tr': 0, 'file_list_header': 0 },
    ...
    product_recommend.init(function () {
      ...
      bt_tools.send({ url: '/files/upload/get_oss_objects',data:{} }, function (list) {
        ...
        $.each(that.cloud_storage_download_list, function (index, item) {
          downHTML+='<li data-type="down_'+item.name+'"><i class="file_menu_icon down_'+item.name+'_file_icon"></i><span  style="position:relative">'+item.title+'<span class="new-file-icon new-ltd-icon" style="right: 26px;top: 4px;left: auto;"></span</span></li>'
        })
  ...
  event_bind: function () {
    ...
    // 数据同步
     $('.file_rsync_list').hover(function () {
          ...
          if (item.name == 'rsync') {
            ...
            if (!item['isBuy'] || !item['install']) {
              if (item['isBuy'] && !item['install']) {
                ...
                product_recommend.recommend_product_view(item,{imgArea: ['994px','535px']},'ltd',70,item.name)
                return false;
              }
            } else {
    ...
    //企业级防篡改
     $('.tamper_core .nav_btn').hover(function () {
        ...
        sName: name
         ...
         if(ltd < 0) {
              ...
              rdata.imgSrc = 'https://www.bt.cn/Public/new/plugin/'+rdata.name+'/1.png'
                product_recommend.recommend_product_view(rdata,{imgArea: ['1000px','610px']},'ltd',75,rdata.name)
    ...
    // 文件解锁/锁定
    $('.file_table_view .file_list_content').on('click', '.file_tamper_core .icon-lock', function (e) {
      var ltd = bt.get_cookie('ltd_end')
    ...
    $('.file_list_content').on('click', '.dir_details', function (e) {
      ...
      bt.soft.get_soft_find('disk_analysis',function(dataRes){
        if(dataRes.setup && dataRes.endtime > 0) {
             _top_text = false
          bt_tools.send({url: '/files/size/get_path_size',data: {path: data.path,is_refresh: false}},function (path_data) {
            pub_details({
              path: data.path,
      ...
      function pub_details(details_data,_top_text) {
          var htmlClassName = ''
					// <span class="firwall_place_of_attribution"></span>
        ...
        <div class="details_cont_item ordinary_cout_item">\
          ...
          <div style="display:flex;align-items:center;width:102px">文件夹数量：</div>\
          <div>'+details_data.dir_num+'</div>\
					</div>\
					<div class="details_cont_item">\
          <div style="display:flex;align-items:center;width:102px">文件数量：</div>\
        ...
        <div class="details_cont_item ordinary_cout_item">\
          ...
          <div>'+ bt.format_data(details_data.mtime) +'</div>\
					...
					<div class="details_cont_item">\
          <div style="display:flex;align-items:center;width:108px">权限变更时间：</div>\
          ...
          $(_this).hover(function(){
            ...
            $(this).parent().find('.details_cont').css({'top':$(this).offset().top,'margin-left':$(this).find('.details_event').width() + 7})
  ...
  reader_file_list_content: function (data, callback) {
    ...
    $.each(data, function (index, item) {
      ...
      _html += '<div class="file_tr" data-index="' + index + '" data-filename="' + item.filename + '" ' + (bt.get_cookie('rank') == 'icon' ? 'title="' + path + '&#13;' + lan.files.file_size + ':' + bt.format_size(item.size) + '&#13;' + lan.files.file_etime + ':' + bt.format_data(item.mtime) + '&#13;' + lan.files.file_auth + ':' + item.user + '&#13;' + lan.files.file_own + ':' + item.root_level + '"' : '') + '>' +
        ...
        '<div class="file_td file_size"><span>' + (item.type == 'dir' ? bt.files.dir_details_span(path,true) : bt.format_size(item.size)) + '</span></div>' +
      ...
      if (typeof item == "boolean") {
        ...
        if (Array.isArray(item)) {
          ...
          $.each(item[1], function (keys, items) {
						$children_list.append($('<li style="display:flex;align-items:center;"><i class="file_menu_icon ' + keys + '_file_icon"></i><span>' + items + '</span><span class="new-file-icon new-ltd-icon '+(items.indexOf('文件')>-1?'hide':'')+'" style="margin-left:40px"></span></li>').on('click', { type: keys, data: config.data }, function (ev) {
								...
								var arrEach = function (arr) {
									$.each(arr, function (index, item) {
										if (item.fullpath == dataPath) {
											...
											if ('subdirectory' in item) arrEach(item.subdirectory)
										}
									})
								}
								arrEach(newPathList)
  ...
  replace_content_view: function () {
    ...
    layer.open({
      ...
      content: '<div class="replace_content_box" style="padding:20px 40px">' +
        '<div class="tab-nav mb15"><span class="on">基础搜索</span><span><i class="glyphicon icon-vipLtd" style="margin: 0 5px 0 0;"></i>高级搜索</span></div>' +
          ...
          '</div>' +
        '<div class="tabs-con-child hide" style="height:539px;position:relative">' +
      ...
      success: function () {
        ...
        $('.replace_content_view .tab-nav span').click(function () {
          var _index = $(this).index(),description = ['文件条件筛选','指定后缀搜索','查找内容展示']
          if($(this).text() === '高级搜索' && bt.get_cookie('ltd_end') < 0){
        //       $('.file-expert-search').addClass('hide')
              var _mask_html = '<div class="mask_layer"><div class="prompt_description" style="height: 46px;width: 264px;line-height: 46px;margin-top: -35px;margin-left: -133px;"><i class="layui-layer-ico layui-layer-ico0" style="width: 20px;height: 20px;display: inline-block;margin-right: 15px;vertical-align: middle;background-size: 700%;"></i>此功能为企业版专享，<a href="javascript:;" class="btlink" onclick="product_recommend.pay_product_sign(\'ltd\', 79, \'ltd\')">立即升级</a></div></div>'
        //       var _html = '<div class="thumbnail-introduce-new" style="margin:0;padding:10px 0 36px">\
        //       <div class="thumbnail-introduce-title-new">\
        //           <div class="thumbnail-title-left-new">\
        //               <div class="thumbnail-title-text-new">\
        //                   <p style="font-size:20px">高级搜索-功能介绍</p>\
        //                 <p style="width:92%;line-height20px;font-size:12px;">支持文件时间条件查找、内容条件查找、后缀查找、以及文件大小查找等，精准定位搜索文件。</p>\
        //               </div>\
        //           </div>\
        //           <div class="thumbnail-title-button-new daily-product-buy" >\
        //              <button class="btn btn-success btn-sm buyNow" style="width:80px">立即购买</button>\
        //           </div>\
        //       </div>\
        ...
        //                         _html_li += '<li><span class="li-icon"></span>'+description[i]+'</li>'
        //                     }
        //                     return _html_li
        //                 })()+'\
        //             </ul>\
        //       </div>\
        //       <img class="img_view" src="/static/images/recommend/uploadFile.png" style="width:300px;height:360px">\
        //   </div>'
            if(bt.get_cookie('ltd_end') < 0) $('.file-expert-search').after(_mask_html)
        //     $('.buyNow').unbind('click').click(function(){
        //         product_recommend.pay_product_sign('ltd', 79, 'ltd')
        //     })
        ...
        //             closeBtn:2,
        //             btn:false,
        //             content:'<img src="/static/images/recommend/uploadFile.png" style="width:100%" />'
        //           })
        //     })
          }else if(bt.get_cookie('ltd_end') > 0){
              $('.replace_content_box .tabs-con-child .mask_layer').remove()
          }
          if (!$(this).hasClass('on')) {
  ...
  check_ltd_end:function ( item,source,config){
      var ltd_cookie = bt.get_cookie('ltd_end')>0
            if(!ltd_cookie){
                product_recommend.recommend_product_view(item, config,'ltd',source,'ltd')
                return false
            }
            return true
  },
  ...
  create_copy_file: function (data) {
    var is_ltd = this.check_ltd_end({ps:'为文件或目录创建副本文件，创建的副本文件将以源文件(1).x命名',description:['可创建文件、目录副本'],preview:false,pluginName:'创建副本',imgSrc:'https://www.bt.cn/Public/new/plugin/introduce/files/copyFile.png'},74,{imgArea: ['1010px','680px']})
    if(!is_ltd) return
    var that = this;
