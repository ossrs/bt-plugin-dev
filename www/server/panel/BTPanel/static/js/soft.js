var soft = {
  ...
  get_list: function (page, type, search) {
    ...
    window.onresize = function () {
      ...
      //     console.log($('.softtype').find('span').attr('typeid'))
      //     $('.softtype').find('span[typeid=0]').siblings().addClass('hide')
      //     $('.softtype').find('span[typeid=-1]').removeClass('hide')
      //     if(!$('.softtype').find('span').hasClass('other')) $('.softtype').append('<span class="other">其他分类</span>')
      // }else if(window.innerWidth >1536){
      //     $('.softtype').find('span').removeClass('hide')
      //     $('.softtype').find('.other').remove()
      // }
  ...
  set_soft_config: function (name) {
    ...
    bt.soft.get_soft_find(name, function (rdata) {
      ...
      var menuing = bt.open({
        type: 1,
        area: (name === 'mysqld' ? ['660px', '632px'] : name.indexOf('php-') > -1 ? ['796px' , '642px'] : "660px"),
