...
var site = {
  ...
  node: {
    ...
    reander_node_project_log: function (el, row) {
      ...
      bt_tools.send({
        ...
        $('#webedit-con .node_project_log').html('<div class="mb10">日志路径 <input type="text" name="model_log_path" id="model_log_path" value="'+res.path+'" placeholder="日志路径" class="bt-input-text mr10 ml5" style="width:350px" />\
        ...
        <div class="mb10" style="line-height: 32px;">日志大小<span class="ml10">'+ res.szie +'</span></div>\
        <pre class="command_output_pre" style="height:500px;">' + res.data + '</pre>')
        $.post({url: '/project/nodejs/get_log_split', data: { name: row.name }}, function (rdata) {
          var html = '',configInfo = {}
          if(rdata.status) configInfo = rdata.data
			    html = '开启后' + (rdata.status ? rdata.data.log_size ? '日志文件大小超过'+ bt.format_size(rdata.data.log_size,true,2,'MB') +'时，进行切割日志文件' : '每天'+ rdata.data.hour +'点' + rdata.data.minute + '分进行切割日志文件' : '默认每天2点0分进行切割日志文件')
          $('#webedit-con .node_project_log .command_output_pre').before('<div class="inlineBlock log-split mb20" style="line-height: 32px;">\
          <label>\
          ...
          $('#webedit-con .node_project_log .log-split').hover(function(){
    ...
    bt.send('GetPHPVersion', 'site/GetPHPVersion', {}, function (rdata) {
      site_table = bt_tools.table({
        ...
        dataFilter:function(res){
          // 数据清空
          trafficType = []
          var thatTrafficName = '总流量',isFlowInfo = !$.isEmptyObject(res.net_flow_info)
          // 流量类型重构
        ...
        column: [{
          ...
          template: function (row, index) {
              ...
              if(!$.isEmptyObject(row.waf) && row.waf.status &&install) return '<div style="display:flex;align-items:center"><span title="WAF防火墙，保护网站安全" class="site_waf_icon_green site_waf_icon" data-name="'+row.name+'" id="site_waf_icon'+ index +'"></span><a class="btlink" href="javascript:;">'+ row.name +'</a></div>'
          ...
          type: 'link',
            ...
            return '<span><a class="btlink site_path" href="javascript:;" title="'+ row.path +'">' + row.path + '</a><a class="btlink site_path hide" href="javascript:;" title="'+ row.path +'">'+ row.path.replace(sites_path,'.../') +'</a></span>';
					},
          event: function (row, index, ev) {
            openPath(row.path);
          },
        ...
        // },
        {
          ...
          template: function (row) {
              //监控是否安装、购买企业版
              if(isLtd && recomConfig.list[1].install){
                // return '<span style="margin-left:24px;max-width:100px;display: inline-block;">' + bt.format_size(row.net[trafficInfo.flow_type]) + '</span>';
								return '<span style="max-width:100px;display: inline-block;">' + bt.format_size(row.net[trafficInfo.flow_type]) + '</span>';
              }else{
                // return '<span style="margin-left:24px;max-width:100px;display: inline-block;" class="btlink traffic_plugin--install">查看</span>';
        ...
        success: function (that) {
          ...
          if(!$('.pull-left button:eq(5) .icon_title').length) {
						...
						$('.pull-left button').eq(5).html('<span class="icon_title" style="position:relative"><span>漏洞扫描</span></span>');
					}
					$('.pull-left button').eq(5).hover(function () {
					...
					$('#bt_site_table .pull-left button').eq(5).before('<div class="inlineBlock dividing-line" style="margin: 0 16px 0 11px;"></div>')
  ...
  init_site_type: function () {
    ...
    $('.tootls_group.tootls_top .pull-right').prepend('<div id="php_cate_select" class="bt_select_updown site_class_type mr10" style="vertical-align: bottom;"><div class="bt_select_value"><span class="bt_select_content">分类:</span><span class="glyphicon glyphicon-triangle-bottom ml5"></span></span></div><ul class="bt_select_list"></ul></div>');
    bt.site.get_type(function (res) {
                  ...
                  success: function (layers, indexs) {
                    ...
                    $('#render_deployment .right-bottom').on('click','.btn-click',function (){
                      ...
                      if(index >= 5){
                        ...
                        dep_mode.eq(index).data('idxs',$(this).data('index')).data('codename',data.name).data('version',data.version).data('versions',data.php).data('title',data.title).data('enable_functions',data.enable_functions)
                ...
                bt.send('SetupPackage','deployment/SetupPackage',pdata,function(res){
                  ...
                  if (res.msg.admin_username != '') {
                    sqlData = "<p class='p1'>已成功部署，无需安装，请登录修改默认账号密码</p>\
                            <p><span>用户：</span><strong>" + res.msg.admin_username + "</strong></p>\
                            <p><span>密码：</span><strong>" + res.msg.admin_password + "</strong></p>\
                            "
                  }
  ...
  get_scan_list: function (callback) {
    ...
    $.post('project/scanning/list', obj, function (res) {
      if (res.status !== false) {
        ...
        if(!$('.pull-left button:eq(5) span.btn_num').length){
					if(!$('.pull-left button:eq(5) .icon_title').length) {
						setTimeout(function(){
							$('.pull-left button:eq(5)').append('<span class="btn_num" style="background:' + (that.scan_num > 0 ? 'red' : '#f0ad4e') + '">' + that.scan_num + '</span>')
						},200)
					}
					$('.pull-left button:eq(5)').append('<span class="btn_num" style="background:' + (that.scan_num > 0 ? 'red' : '#f0ad4e') + '">' + that.scan_num + '</span>')
				}else{
					$('.pull-left button:eq(5) span.btn_num').css('background', that.scan_num > 0 ? 'red' : '#f0ad4e').html(that.scan_num);
				}
				if (callback) callback(res);
      }else{
        that.scan_list = res
  ...
  edit: {
        ...
        bt.site.get_dir_userini(web.id, path, function (rdata) {
          ...
          var quota_data = [
            ...
            },
            {
              title: '',
              items: [
                { title: '<span class="glyphicon icon-vipLtd" style="margin-left: -23px;"></span> 网站配额容量', type: 'number', width: '240px',min: '0', value: getQuotaData_size(web), name: 'quota_sized', unit: '<span class="used_unit">MB</span>' },
                {
                  ...
                  callback: function (pdata) {
                    ...
                    bt.public.modify_path_quota({ data: JSON.stringify({ size: quota_size, path: web.path }) }, function (res) {
                      if (res.status) {
                        ...
                        bt_tools.send({url:'/data?action=getData',data:{  table: 'sites' }},function(rdata){
													for(var i = 0;i < rdata.data.length;i++){
          ...
          _html.append('<div class="user_pw_tit" style="margin-top: 10px;padding-top: 11px;"></div><div class="quota_block" style="position:relative"></div>')
          if (bt.os == 'Linux') _html.append('<div class="user_pw_tit" style="margin-top: 10px;padding-top: 11px;"><span class="tit">密码访问</span><span class="btswitch-p"><input class="btswitch btswitch-ios" id="pathSafe" type="checkbox"><label class="btswitch-btn phpmyadmin-btn" for="pathSafe" ></label></span></div><div class="user_pw" style="margin-top: 0px; display: block;"></div>')
          ...
          $('.quota_block').append(bt.render_help(['需要XFS文件系统，且包含prjquota挂载参数才能使用','fstab配置示例：/dev/vdc1 /data xfs defaults,prjquota 0 0','配额容量：设置当前网站最大容量，如需取消容量配额，请设为“0”']));
          bt.render_clicks(clicks);
					if(bt.get_cookie("ltd_end")<0){
      ...
      bt.site.get_site_phpversion(web.name, function (sdata) {
        ...
        bt.site.get_all_phpversion(function (vdata) {
          ...
          if (sdata.phpversion != 'other') {
            $('#webedit-con').append('<div class="user_pw_tit" style="margin-top: 22px;padding-top: 28px;border-top: #ccc 1px dashed;"><span class="tit">session隔离</span><span class="btswitch-p" style="display: inline-block;vertical-align: middle;"><input class="btswitch btswitch-ios" id="session_switch" type="checkbox"><label class="btswitch-btn session-btn" for="session_switch" ></label></span></div><div class="user_pw" style="margin-top: 10px; display: block;"></div>' +
          ...
          $('#webedit-con').append('<div class="user_pw_tit php-pro-box" style="position: relative;margin-top: 22px;padding-top: '+padding_num+'px;border-top: #ccc 1px dashed;">\
            ...
            </div>'+bt.render_help(['基于PHP内核的监控工具，实时监控网站木马、漏洞等其他入侵行为，发现木马支持自动隔离','安全告警默认会监视当前网站访问非当前网站文件的行为(例：你的站点A.com访问了站点B.com的文件或者目录)，通过添加监视器路径，当路径被访问时发送告警'])+
          '</div>')
          //判断不兼容版本
            ...
            bt.soft.get_soft_find('security_notice',function(rdata_res){
              ...
              if(!rdata_res.setup || rdata_res.endtime < 0){
                ...
                if(rdata_res.endtime >=0){
                  $('#webedit-con .php-pro-box').append('<div class="mask_layer">\
              ...
              if(rdata_res.setup && is_install) {
                //获取全局防护状态
                bt_tools.send({ url: '/plugin?action=a&name=security_notice&s=get_status',data:{}},function(global_config){
                  bt_tools.send({ url: '/plugin?action=a&name=security_notice&s=get_index',data: {}},function (index_res) {
                    var php_versions = index_res.php_versions ? index_res.php_versions.filter(function (a){
                    ...
                    if((php_versions.length > 0 && php_versions[0].state !== 1) || !global_config.send) {
                      ...
                      $('.sn-home-cont').append('<div class="sn-home--open-condition"><i class="sn-home--important-note">!</i>未启用PHP安全告警，请先'+ (!global_config.send ? '<span class="set_send">设置告警</span>' : '') + ((php_versions.length > 0 && php_versions[0].state !== 1) && !global_config.send ? '和' : '') + ((php_versions.length > 0 && php_versions[0].state !== 1) ? '开启<span class="php_status" data-version="'+ sdata.phpversion +'">PHP-'+ sdata.phpversion +'防护</span>' : '' ) +'</div>')
                    }
                    bt_tools.send({ url: '/plugin?action=a&name=security_notice&s=get_sites',data:{}},function (site_res) {
                      var site_filter = site_res.sites ? site_res.sites.filter(function (a) {
                        return a.path === web.path && a.site_name === web.name
                      }) : []
                      if(site_filter.length  > 0) {
                        var site_data = site_filter[0]
                        $('.today-num span').text(site_data.total.day_total).addClass(site_data.total.day_total > 0 ? 'color-red':'') //今日
                        $('.all-num span').text(site_data.total.total).addClass(site_data.total.day_total > 0 ? 'color-red':'') //总告警数
                        ...
                        $('.php-pro-box .sn-home--open-condition .php_status').click(function () {
                          ...
                          bt.simple_confirm({
                            ...
                            bt_tools.send({ url: '/plugin?action=a&name=security_notice&s=set_php_status',data: {php_version:_version,enable: 1}},function(rdata){
                              loadT.close();
                              if(rdata.status) {
                                site.reload();
                              }
                        ...
                        $('.set_send').on('click',function(){
                          bt.open({
                            ...
                            success: function() {
                              var sendChannel = [{name:'邮件',value:'mail'},{name:'微信公众号',value:'wx_account'},{name:'企业微信',value:'weixin'},{name:'钉钉',value:'dingding'},{name:'飞书',value:'feishu'}]
                              // 获取告警列表
                              bt_tools.send({
                                ...
                                bt_tools.send({url: '/plugin?action=a&name=security_notice&s=get_send_config',data: {}},function(rdata){
                                  ...
                                  $('.php_set_send .sn-alarm--list').find('.installNotice').unbind('click').click(function (ev) {
                        ...
                        $('#isPhpproSwitch').on('change',function(){
                          ...
                          bt.simple_confirm({
                            ...
                            bt_tools.send({ url: '/plugin?action=a&name=security_notice&s='+ (_status?'stop_site':'start_site'),data: {siteName:site_data.site_name}},function(rdata){
                              loadT.close();
                              if(rdata.status) {
                        ...
                        //监视器
                        $('.edit_file_monitor').unbind('click').click(function () {
                          bt_tools.open({
                            type: 1,
                                          ...
                                          edit_file_monitor()
																...
																function del_file_monitor(items){
																	bt.confirm({
																		...
																		bt_tools.send({url: '/plugin?action=a&name=security_notice&s=del_site_config',data:{domain:site_data.site_name,type: items.type,path: items.path}},function (rdata) {
																			bt_tools.msg(rdata)
																			if(rdata.status) {
                        ...
                        //触发日志
                        $('.get_domain_logs').unbind('click').click(function () {
    ...
    templet_301: function (sitename, id, row) {
      ...
      bt.site.get_domains(id, function (rdata) {
        ...
        var content = '<style>select.bt-input-text { width: 100px; }</style>\
				<div id="form_redirect" class="bt-form-new pd20">\
					...
					<div class="form-inline redirectpath" style="display: none; flex-wrap: nowrap;">\
						<div class="form-item">\
							...
							<div class="form-value">\
								<input class="bt-input-text" name="redirectpath" type="text" style="width: 200px;" />\
							</div>\
						</div>\
						<div class="form-item">\
							<div class="form-label" style="width: 80px;">目标URL</div>\
		...
		templet_404: function (sitename, row) {
			...
			bt.open({
				...
				btn: ['提交', '取消'],
				content: '<style>.form-item .form-label { width: 132px; } .form-item .bt-input-text { width: 180px; } </style>\
				<div class="bt-form-new pd20" style="padding-bottom: 25px;">\
					...
					<div class="form-item">\
						...
						<div class="form-value">\
							...
							<input type="text" name="tourl" class="bt-input-text mt10" style="display: none;" placeholder="请输入自定义页面路径" />\
						</div>\
					</div>\
				</div>',
				success: function () {
				...
				yes: function (index) {
					...
					bt.site.set_error_redirect(isEdit, {
          ...
          },
          {
            ...
            },
					{
						...
						template: function (row, index) {
							...
							return '<span title="' + path + '" style="display: flex;">\
								<span class="size_ellipsis" style="flex: 1; width: 0;">' + (row.topath && path == '/' ? '首页' : path) + '</span>\
							</span>';
            }
          },
          {
    ...
    security_scanning: function (web) {
      ...
      $.post('project/webscanning/ScanSingleSite', { data: JSON.stringify({ "name": siteName }) }, function (res) {
        ...
        if (res.status) {
          ...
          $('.security-sacn-body').html('<div class="thumbnail-introduce-new" style="margin:0;padding:0px 0">\
      ...
      try {
        if (!_config['isBuy'] || !_config['install']) {
          $.each(_config['product_introduce'], function (index, item) {
					...
					recom_Template = '<div class="thumbnail-introduce-new recommend" style="margin:0;padding:10px 0 10px">\
              <div class="thumbnail-introduce-title-new" style="width:100%;justify-content:normal;flex-direction:column;padding-left: 24px;">\
                  <div class="thumbnail-title-left-new" style="width:100%;">\
                      <img src="/static/img/soft_ico/ico-tamper_proof.png" alt="图标" class="'+(_config['title']==='网站防篡改程序'?'':'hide')+'" style="width:62px"/>\
                      <div class="thumbnail-title-text-new">\
                          <p>' + _config['title'] + '-功能介绍</p>\
                        	<p>'+_config['ps']+'</p>\
                      </div>\
                  </div>\
                  <div class="thumbnail-title-button-new daily-product-buy" style="'+(_config['title'].indexOf('证书')>0?'margin:10px 0 0 0':'margin:16px 0 0 76px')+'">\
              ...
              <div class="img_view" style="position: relative;">\
                <img class="product_view_img" src="'+_config.previewImg+'" style="height:'+(_config['title']==='网站防篡改程序'?'440':'400')+'px; margin-top:10px;box-shadow: 1px 1px 30px rgb(0 0 0 / 10%);"/>\
                <div class="img_view_mask"></div>\
							</div>\
            </div>'
  ...
  render_domain_result_table: function (data) {
    ...
    $.each(data.domains, function (index, item) {
      _num++;
      resultHTML += '<tr><td><span class="text-overflow" title="' + item.name + '">' + item.name + (noSSL ? ($.inArray(item.name, data.not_ssl) != -1 ? '<span class="bt-ico-ask" title="非当前域名的泛域名，无法直接使用当前站点部署的ssl证书的域名">!</span>' : '') : '') + '</span></td><td><span style="float:right; color:' + (item.status ? '#20a53a' : 'red') + '">' + item.msg + '</span></td></tr>'
    })
    bt.open({
      type: 1,
      title: '添加域名结果',
              ...
              if (udata.status) {
                ...
                function pay_ssl_business () {
                  ...
                  bt.send('get_product_list_v2', 'ssl/get_product_list_v2', {}, function (res) {
                    loadT.close();
                    var dataLength = res['data'] && res.data.length,data_list = res.data,list = [],prompt_msg = res.info
                    bt.open({
                      type: 1,
                      title: '购买商业证书',
                      area: ['790px','829px'],
                      ...
                      success: function (layero, indexs) {
                        ...
                        function reader_product_info (config) {
                          ...
                          var year_list = ylist.filter(function(s){return p_index ? s.code.indexOf('wildcard') > -1 : s.code.indexOf('wildcard') === -1 })
                          var is_flag = year_list.some(function (s){return s.code.indexOf('multi') > -1 || s.brand === 'Digicert' })
                          if(p_index) {
                          ...
                          if(input.val() > 1) {
                            // disabled
                            //证书类型禁用
                            var is_type_disabled = ylist.filter(function(s){return !p_index ? s.code.indexOf('wildcard') > -1 : s.code.indexOf('wildcard') === -1 }).some(function (s){return s.code.indexOf('multi') > -1 || s.brand === 'Digicert' })
                            if(!is_type_disabled) {
                          ...
                          //选中的证书信息
                          var data_info = year_list.filter(function(s){return $('.domain_number_input').val() > 1? (s.brand === 'Digicert' ?  s.code.indexOf('multi') === -1 : s.code.indexOf('multi') > -1) : s.code.indexOf('multi') === -1})[0]
                          dataInfo = data_info
                          ...
                          $('.business_ssl_btn .bname span').html('默认包含'+ data_info.num +'个域名 '+ price_pack +'元/'+ year +'年'+(cur_num ? '，额外超出'+cur_num+'个域名'+ price_extra +'元/'+ year +'年':''))
                        ...
                        ssl_type_item.click(function() {
                          ...
                          var type = $(this).data('type'),
                              brand_list = []//品牌类型
                          list = dataLength ? data_list.filter(function (s) {return s.type.indexOf(type) > -1;}) : []
                          var type_tips_list = prompt_msg['type'][type.toLowerCase()],type_tips = ''
                          ...
                          for (var i = 0; i < brand_list.length; i++) {
                            ...
                            var b_list = list.filter(function (s){return s.brand === brand_list[i]})
                            //品牌是否存在多域名
                            var is_multi = b_list.some(function(s){return (s.code.indexOf('wildcard') === -1 && s.code.indexOf('multi') > -1) || s.brand === 'Digicert'})
                            var is_multi_w = b_list.some(function(s){return (s.code.indexOf('wildcard') > -1 && s.code.indexOf('multi') > -1) || s.brand === 'Digicert'})
                        ...
                        ssl_brand_item.click(function () {
                          ...
                          if(!$(this).hasClass('active')) $(this).addClass('active').siblings().removeClass('active');
                          var type = $(this).data('type'),years_list = [],max_years = 0,years_html = '',cert_html = ''
                              ylist = list.filter(function (s) {return s.brand.indexOf(type) > -1;})
                              brand_type = $('.ssl_type_item.active').data('type')
                          var brand_tips_list = prompt_msg['brand'][type==='Positive' || type === '锐安信' ? type:type.toLowerCase()],brand_tips = ''
                          ...
                          //是否存在单域名/泛域名按钮
                          is_single = ylist.some(function(s){return s.code.indexOf('wildcard') === -1})
                          is_worldxml = ylist.some(function(s){return s.code.indexOf('wildcard') > -1})
                          if(is_single){
                        ...
                        $('.business_ssl_form').on('click','.ssl_price_item',function(){
                          ...
                          if(!$(this).hasClass('active')) $(this).addClass('active').siblings().removeClass('active');
                          var price_tips_list_0 = ['仅支持绑定一个二级域名或者子域名，例如 bt.com、cloud.bt.com、dnspod.cloud.bt.com的其中之一 。','如需要绑定同级的所有子域名，例如*.bt.com，请购买泛域名证书。'],
                              price_tips_list_1 = ['带通配符的域名，例如：*.bt.com、*.cloud.bt.com均为泛域名，包含同一级的全部子域名。','注意泛域名不支持跨级，例如*.bt.com 不包含 *.cloud.bt.com的支持']
                          var price_tips_list = $(this).index() ? price_tips_list_1 : price_tips_list_0,price_tips = ''
                        ...
                        $('.business_ssl_form').on('click','.ssl_year_item',function(){
                          if(!$(this).hasClass('active')) $(this).addClass('active').siblings().removeClass('active');
                          year = $(this).data('year')
                          var year_tips_list = prompt_msg['times'][year+'_year'],year_tips = ''
                        ...
                        ssl_service_item.click(function () {
                          ...
                          ssl_service_tips.html(index ? index === 1 ? '宝塔提供9:00 - 24:00的人工部署证书部署服务，帮助客户排查部署证书部署生效问题，快速上线' : '宝塔提供9:00 - 24:00的人工部署国密算法证书部署服务，帮助客户排查部署证书部署生效问题，快速上线' : '')
                        ...
                        $('.order_pay_btn a').click(function () {
                          switch ($(this).data('type')) {
                            case 'info':
                              confirm_certificate_info($.extend(dataInfo, {
                                oid: order_id,
                                qq: qq_info,
                        ...
                        function reader_applay_qcode (data, callback) {
                          ...
                          $('.lib-price-detailed .info:eq(1) span:eq(1)').html(data.time);
                          if (typeof data.qq != "undefined"){
                            $('.order_pay_btn a:eq(0)').click(function(){bt.onlineService()})
                          } else {
                            $('.order_pay_btn a:eq(0)').remove();
                          }
                          if (callback) callback(data);
