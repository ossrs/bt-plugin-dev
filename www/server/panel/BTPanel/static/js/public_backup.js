	...
	dir_details: function () {
		$('.dir_details').unbind('click').click(function(e){
        	...
        	if(rdata1.setup && rdata1.endtime > 0) {
        		$.post({url: '/files/size/get_path_size',data: {path: path,is_refresh: is_refresh ? is_refresh : false}},function (res) {
						  if(res['status'] === false) {
	              if(res['code'] && res['code'] === 404){
	            	...
	            	$this.find('.details_cont_load').hide()
								// var item = res[path.lastIndexOf('/') == path.length - 1 ? path.substring(0,path.length - 1) : path]
								var item = res[path]
								...
								var mtime = bt.format_data(item.mtime);//修改时间
								var stime = bt.format_data(item.stime);//扫描时间
								var ctime = bt.format_data(item.ctime);//权限修改时间
								var html = '<div class="details_cont_item"><span>目录详情<span>'+ stime +'</span></span><button type="button" title="刷新" class="btn btn-default btn-sm details_refresh"><i class="icon-refresh"></i><span>刷新</span></button>\
...
bt.soft = {
		...
		layer.open({
			...
			success: function (layers, indexs) {
				...
				bt.send('get_coupons','auth/get_coupons',{},function(res_coupons){
						if(res_coupons === null) return get_coupons()
            ...
            var ops_obj = {
              ...
              rlist: [
                ...
                'CPU内存占用100%处理',
						...
						if (!config.plugin) arry.push(ops_obj);
	...
	set_pay_status_icon:function(){
		...
		bt.send('get_pd', 'ajax/get_pd', {}, function (res) {
			...
			if(!is_pay){
				proHTML = '<span class="btltd-gray" onclick="bt.soft.updata_ltd()" style="margin-right:4px" title="点击升级到企业版">免费版</span>'
