var bt_tools = {
	...
	table: function (config) {
      ...
      cache_set_all: {},//所有缓存设置显示隐藏的列数据
        ...
        /*** 渲染设置显示隐藏列按钮 ***/
        var setting_btn = this.config.setting_btn
        if (setting_btn) {
          var cache_name = this.config.el + '_' + _that.random,setting_ul_html = ''
          this.cache_set_column = []
          var flag = false
          for (var j = 0; j < column.length; j++) {
            if(column[j]['type'] == 'checkbox') continue;
            if(!column[j]['width']) flag = true
            this.cache_set_column.push({
          ...
          if(this.config.hasOwnProperty('setting_list')) {
            for (var j = 0; j < this.config.setting_list.length; j++) {
              this.cache_set_column.unshift(this.config.setting_list[j])
            }
          }
          this.cache_set_all = bt.get_storage('cache_set_all') ? JSON.parse(bt.get_storage('cache_set_all')) : this.cache_set_all;// 获取所有缓存数据
          this.cache_set_column = this.cache_set_all[this.config.el] ? this.cache_set_all[this.config.el] : this.cache_set_column;  // 有缓存时用缓存数据

          // 渲染显示隐藏列按钮下拉列表
          for (var j = 0; j < this.cache_set_column.length; j++) {
            var obj = this.cache_set_column[j],title = $(obj.title).text();
            setting_ul_html += '<li class="setting_ul_li'+ (obj.value ? ' active' : '') + (obj.disabled ? ' disabled' : '') +'">\
              <i></i>\
          ...
          //渲染显示隐藏列按钮
          if(!$(this.config.el + ' .set_list_fid_dropdown').length) {
            $(this.config.el + ' .tootls_top .pull-right').append('<div class="set_list_fid_dropdown"><div class="setting_btn"><i class="glyphicon glyphicon-cog icon-setting"></i></div>\
              <ul class="setting_ul">'+setting_ul_html+'</ul></div>')
          }
          //显示隐藏列鼠标移入事件
          var set_interval = null
          $(this.config.el + ' .setting_btn').hover(function(){
            $(this).next().show()
          },function(e){
          ...
          //显示隐藏列点击事件
          $(this.config.el + ' .setting_ul .setting_ul_li').unbind('click').click(function(e){
            if($(this).hasClass('disabled')) return false;
            if($(this).hasClass('active')) $(this).removeClass('active')
            else $(this).addClass('active')
            var index = $(this).index();
            _that.cache_set_column[index].value = !_that.cache_set_column[index].value; //重置显示状态
            _that.cache_set_all[_that.config.el] = _that.cache_set_column;
            bt.set_storage('cache_set_all',JSON.stringify(_that.cache_set_all)) //设置本地存储所有数据缓存
            _that.init = false; //重置表头和数据
            _that.style_list = [_that.style_list[0]]; //重置样式
            $('#bt_table_' + _that.random).remove();  //重置表样式
            _that.$reader_content(data, page); //刷新数据
          })
        }
        /*** -------end-------- ***/
            ...
            /*** -------end-------- ***/
				...
				if (!this.init) {
          $(this.config.el + ' .divtable .table').remove()
        ...
        /*** 设置列显示隐藏---单独某个class处理 ***/
        if(setting_btn && this.config.hasOwnProperty('setting_list')) {
          for (var j = 0; j < this.cache_set_column.length; j++) {
            var obj = this.cache_set_column[j]
            if (obj.hasOwnProperty('class')) {
              if(obj.value) $(this.config.el + ' .' + obj.class).show()
              else $(this.config.el + ' .' + obj.class).hide()
            }
          }
        }
        /*** -------end-------- ***/
