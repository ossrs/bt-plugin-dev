...
var index = {
  ...
  get_init: function () {
    ...
    var create_data_table = function (el, type, config) {
      bt_tools.send({url: '/monitor/process_management/specific_resource_load_type'}, function (res) {
          ...
          // }
				...
				if(res.info){
					$('#load_cpu_times_num').text(res.info.total_processes+ '  /  ' + res.info.active_processes)
				}
     ...
     */
      ...
      if(is_other){//cpu使用率 其他信息
        for (var i = 0; i < config.html.cpu_times.length; i++) {
          other_table_html += '<div class="disk-body-list">\
            <span>核心 '+ (i+1) + (config.html.cpu_times[i+1] ? ' / ' + (i + 2) : '' ) +' 占用：</span>\
            <span id="cpu_'+ (i+1) +'_num">'+ config.html.cpu_times[i] +'%' + (config.html.cpu_times[i+1] ? ' / ' + config.html.cpu_times[i+1] + '%' : '' ) +'</span>\
          </div>'
          i++
        }
      }else{//负载状态 其他信息
      ...
      layer_id = layer.open({
        ...
        content:
          '<div class=" load_tips disk_cont">\
            ...
            <div class="disk-item-list active base_html" id="base_info">\
              ...
              (config.html.isShow ? '<div id = "other">\
                <div class="disk-item-list '+ (is_top_active ? 'active' : '') +'" id="top5_info">\
                  ...
                  <div className="disk-header-fold"><span>'+ (is_top_active ? '折叠' : '展开') +'</span><img src="/static/img/arrow-down.svg" '+ (is_top_active ? 'style="transform: rotate(180deg);"' : '') +'></div>\
                </div>\
                <div class="disk-item-body disk-item-body-nottom" style="position:relative">\
                  <div class="disk-body-list no-bg">\
									...
									<div class="un-profession" style="display:none">\
        ...
        success: function (layero, index) {
          // <div class="disk-item-footer" style="display:none">\
					// 				<a id="task_manager_btn" style="margin:auto" title="查看更多进程信息" class="btlink"">查看更多进程信息</a>\
					// 	    </div>\
          //预渲染一个空表格占位
          ...
          float_box({
            ...
            html: {
              ...
              base_html:
                ...
                <div class="disk-body-list">\
                  <span>总进程数 / 活动进程数：</span>\
                  <span id="load_cpu_times_num">' + rdata.cpu_times['总进程数'] + '  /  ' + rdata.cpu_times['活动进程数'] + '</span>\
                </div>',
            ...
            html: {
              base_html: '<div class="disk-body-list">\
                ...
                <div class="disk-body-list" title="由内核自动分配buffer/cache缓存大小，以提高文件系统性能和整体系统的响应速度">\
                  <span>Buffer / Cache内存占用：</span>\
                  <span id="mem_buffer_cache">' + (rdata.mem.memBuffers + rdata.mem.memCached) + '</span>\
                </div>',
              slot: '<div class="mem-title">\
                            ...
                            <div class="disk-item-list '+ (is_active ? 'active' : '') +'">\
                              <div class="disk-item-header">\
                                ...
                                <div class="disk-header-fold">\
                                  <span>'+ (is_active ? '折叠' : '展开') +'</span>\
                                  <img src="/static/img/arrow-down.svg" '+ (is_active ? 'style="transform: rotate(180deg);"' : '') +'>\
                                </div>\
															</div>\
                              <div class="disk-item-body">\
    ...
    setTimeout(function (){
      if(window.navigator.userAgent.indexOf('MSIE ')>-1||window.navigator.userAgent.indexOf('Trident/')>-1){
        $('.btltd-gray').css('padding-left','130px')
      }
    },500)
  },
  ...
  reander_system_info: function (callback) {
    ...
    bt.system.get_net(function (res) {
      ...
      //实时刷新负载弹窗数据
      $('.load_text').text((loadCount >= 80 ? '运行不稳定' : '运行流畅')).removeClass('color-red color-org color-green').addClass((loadCount >= 80 ? (loadCount >= 90 ? 'color-red' : 'color-org') : 'color-green'))
      ...
      for (var i = 0; i < res.cpu[2].length; i++) {
        $('#cpu_'+ (i+1) +'_num').text(res.cpu[2][i] + '%' + (res.cpu[2][i+1] ? ' / ' + res.cpu[2][i+1] + '%' : ''))
        i++
      }
      ...
      $('#mem_buffer_cache').text((res.mem.memBuffers + res.mem.memCached))
  ...
  get_index_list: function () {
    bt.soft.get_index_list(function (rdata) {
      ...
      //交互优化------普通用户 首页软件推荐增加近3天不再显示
      if(bt.get_cookie('ltd_end') < 0 && bt.get_cookie('pro_end') < 0) {
            ...
            layer.open({
              ...
              success: function (layero, index) {
                ...
                $('.openLtd').click(function () {
                  product_recommend.pay_product_sign('ltd', 68, 'ltd')
                })
              },
            });
          });
