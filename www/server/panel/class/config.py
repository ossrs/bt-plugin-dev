 ...
 # 取Session缓存方式
    ...
    def set_ssl_verify(self,get):
        ...
        if 'crl' in get and 'ca' in get:
            ...
            if not public.check_ssl_verify("ssl/crl.pem"):
                return public.returnMsg(False, 'key错误,请检查!')
            if not public.check_ssl_verify("ssl/ca.pem"):
    ...
    def get_nps(self, get):
        ...
        if os.path.exists("data/%s_nps_time.pl"%software_name):
            try:
                ...
                data['safe_day'] = float((time.time() - nps_time) / 86400)
