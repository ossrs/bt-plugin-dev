...
class main(projectBase):
    ...
    def install_tomcat(self,get):
        ...
        if version=="7":
            if os_ver=='Ubuntu':
                return public.returnMsg(False,'操作系统不支持!')
        public.ExecShell("rm -rf /tmp/1.sh && /usr/local/curl/bin/curl -o /tmp/1.sh http://download.bt.cn/install/src/webserver/shell/new_jdk.sh && bash  /tmp/1.sh %s %s >>%s"%(get.type,version,tmp_file))
    ...
    def get_project_log(self,get):
        ...
        if project_info['project_config']['java_type']=='springboot':
            ...
            if log_file_size>3145928:
                return {"status" : True, "path": _path, "data" : self.xsssec(self.last_lines(log_file,3000)), "size": public.to_size(log_file_size)}
            return {"status" : True, "path": _path, "data" : self.xsssec(public.GetNumLines(log_file,3000)), "size": public.to_size(log_file_size)}
        if project_info['project_config']['java_type']=='duli':
...
#—————————————
    ...
    def _update_project(self, project_name, project_info):
        ...
        # 保存
        if project_info['project_config']['java_type']=='springboot':
            target_file = self._springboot_logs_path + "/" + project_name + ".log"
            log_file = project_info['project_config']['logs']
