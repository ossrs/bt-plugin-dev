...
class SiteDirAuth:
    ...
    def set_dir_auth(self, get):
        ...
        name_err, dir_err = self._check_dir_auth(site_name, name, site_dir)
        if name_err:
    ...
    def _check_dir_auth(self, site_name, name, site_dir):
        ...
        if not conf:
            return False, False
        for i in conf.get(site_name, list()):
            name_err =  name in i.values()
            dir_err =  site_dir == i["site_dir"]
            if name_err or dir_err:
                return name_err, dir_err
        return False, False
