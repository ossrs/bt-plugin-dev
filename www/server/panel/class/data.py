...
class data:
    __ERROR_COUNT = 0
    #自定义排序字段
    __SORT_DATA = ['site_ssl','php_version','backup_count','total_flow','7_day_total_flow','one_day_total_flow','one_hour_total_flow']
    DB_MySQL = None
    web_server = None
        ...
        try:
            ...
            if table == 'backup':
                ...
                for i in range(len(data['data'])):
                    if data['data'][i]['size'] == 0:
                        ...
                        if not os.path.exists(data['data'][i]['filename']):
                            if (data['data'][i]['filename'].find('/www/') != -1 or data['data'][i]['filename'].find(
                                    backup_path) != -1) and data['data'][i]['filename'][0] == '/' and data['data'][i][
                                'filename'].find('|') == -1:
                                data['data'][i]['size'] = 0
                ...
                for i in range(len(data['data'])):
                    # 清除不存在的
                    backup_count = 0
                    backup_list = SQL.table('backup').where("pid=? AND type=?", (data['data'][i]['id'], type)).select()
                    for backup in backup_list:
                        if len(backup["filename"].split("|")) !=3 and  not os.path.exists(backup["filename"]):
                            public.M('backup').where("id=? AND type=?", (backup['id'], type)).delete()
                            continue
                        backup_count += 1
